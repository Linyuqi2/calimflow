...
for i = 1:imax
  for j = 1:jmax
    ...
    if (mod(N,2)~=0)&&(mod(N2,2)==0); N2 = N2-1; end  % ASC ADDED - Fixes a bug if N - N2 is odd which makes later sizes of arrays incompatible
    ...
    UoutB = padarray(UoutB,double([1 1]*((N-N2)/2)),0,'both');  % FLOOR OR CEIL?? -ASC
