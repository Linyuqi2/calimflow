...
if(acc_flag)
  ...
  elseif strcmp(psf_params.type,'cylindrical')
    Uin = generateCylindricalBA(vol_params,psf_params);
  else
...
if ~isstruct(Uin)
  if(psf_params.fastmask)
    ...
    elseif strcmp(psf_params.type,'cylindrical')
      UinFM = generateCylindricalBA(vol_params,psf_paramsFM);
    else
      warning('Needs to be a supported type, defaulting to ''gaussian''')
      UinFM = generateBA(vol_params,psf_paramsFM);
    end
...
colmask = colmask(floor((colpx(1)-volpx(1))/2)+(1:volpx(1)), ...
                               floor((colpx(2)-volpx(2))/2)+(1:volpx(2))); % 
