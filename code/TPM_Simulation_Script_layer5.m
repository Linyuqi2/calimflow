...
save_path            = '/jukebox/pillow/adam/naomi_sim/results/';          % Set the directory to save results to
save_filename        = 'layer5vol';                                        % Pick a file to save the simulated movie as (ends with either .fits, .tif, or .mat)
vol_params.vol_sz    = [150,150,100];                                      % Volume size to sample (in microns)
vol_params.vol_depth = 500;                                                % Set the depth of imaging (depth at the middle of the simulated volume)
...
spike_opts.nt        = 5000;                                              % Set number of time step
...
scan_params.motion   = true;                                               % Toggle this parameter to turn motion simulation on/off


% vol_params.neur_density = scaleDen*1e5;                                    % Set neuron density
neur_params.avg_rad     = scaleVal*5.9;                                    % Default average radius is 5.9: scale up by ~20% to simulate closer to layer 5
vol_params.min_dist     = 16 + 2*(scaleVal-1)*neur_params.avg_rad;         % Calculate a new minimum distance between neurons
vol_params.AD_density   = 2e2;
vol_params.N_den        = ceil(vol_params.AD_density*prod(vol_params.vol_sz(1:2))/(1e6));
vol_params.neur_density = 7e4;
vol_params.N_neur       = ceil(vol_params.neur_density*prod(vol_params.vol_sz)/(1e9));
axon_params.maxfill     = 0.4;
dend_params.atParams    = [4 5 5 5 2];  
...
scan_params  = check_scan_params(scan_params);                             % Check the scanning parameter struct
...
save([save_path,save_filename,'.mat'],'-v7.3')                             % Saves all variables to a .mat file
