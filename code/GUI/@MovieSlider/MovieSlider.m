...
%     CONTRAST_RANGE        = 6e-10 * (1:1e2:1e3).^3
  ...
  properties (Access = protected)
    ...
    txtInfo
    btnConfig
    % MODDED - changed accessibility of the following properties
    % btnPlay
    % btnRepeat
    % btnBegin
    % btnEnd
  ...
  properties (SetAccess = protected)
    ...
    colors
    doRepeat              = false
    % MODDED - changed accessibility of currentFrame
    % currentFrame        = 1
    contrastIndex         = 1
  ...
  methods
    ...
    %----- Constructor
    function obj = MovieSlider(parent, spike, varargin)
      ...
      % MODDED - assign spike_opts value
      obj.spike_opts = spike;
      % END MODDED
      
      % MODDED - set axsMovie Parent Background Color
      %% Movie frame display
      obj.axsMovie      = axes      ( 'Parent'                  , uicontainer('Parent', obj, 'BackgroundColor', [1 1 1])                    ...
      ...
      obj.figParent     = get(parent, 'parent');
      ...
      %% Playback timer
      % MODDED - create timePeriod variable to change MovieSlider play
      %          speed
      timePeriod = round(obj.spike_opts.dt,1);
      if (timePeriod == 0)
          timePeriod = 0.1;
      end
      % END MODDED
      
      % MODDED - add Period to tmrPlayback
    ...
    function show(obj, movie, colors, pixelDomain, contrast, contrastIndex)
      ...
      if obj.totalFrames >= MovieSlider.MIN_NUMBINS * MovieSlider.CONTRAST_BINNING
          %PROBLEM
          binnedMovie   = obj.movie;%rebin(obj.movie, MovieSlider.CONTRAST_BINNING, 3, @mean, 'omitnan');
      else
...
%         obj.pixelRange      = interp1(obj.pixelCDF(sel), obj.pixelValue(sel), [saturation, 1-saturation], 'linear', 'extrap');
  ...
  methods (Access = protected)
    ...
    function doDrawFrame(obj, iFrame)
      ...
      set(obj.txtInfo, 'String', sprintf('%.1f/%.1f', ...
                        round(iFrame*obj.spike_opts.dt, 1), ...
                        obj.spike_opts.nt*obj.spike_opts.dt));
...
%PROBLEM?
        xRange          = testrangemin(get(movieSliders(iMov).axsMovie, 'XLim'), [1, size(movieSliders(iMov).movie,2)] );
        yRange          = testrangemin(get(movieSliders(iMov).axsMovie, 'YLim'), [1, size(movieSliders(iMov).movie,1)] );
          mousePos      = pixel;
        else
