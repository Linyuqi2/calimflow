...
class VasculatureSimulator:
    ...
    def simulate(self) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        ...
        self.vasc_params._depth_vasc = self.vasc_params.depth_vasc * self.vol_params.res    # TODO: Deprecated
        ...
        self.vasc_params._size = (self.vol_params.size + np.array([0, 0, self.vol_params.depth])) * self.vol_params.res
        self.vasc_params._szum = self.vasc_params._size + np.array([0, 0, self.vol_params.depth])
        self.vasc_params._nsource = max(round((2 * (self.vol_params.size[0] + self.vol_params.size[1]) / self.vasc_params.source_freq) * abs(1 + self.vasc_params.ves_num_scale * np.random.randn())), 0)
        self.vasc_params._nvert = max(round((self.vol_params.size[0] * self.vol_params.size[1] / (self.vasc_params.ves_freq[1]**2)) * abs(1 + self.vasc_params.ves_num_scale * np.random.randn())), 0)
        self.vasc_params._nsurf = max(round((self.vol_params.size[0] * self.vol_params.size[1] / (self.vasc_params.ves_freq[0]**2)) * abs(1 + self.vasc_params.ves_num_scale * np.random.randn())), 0)
        self.vasc_params._ncapp = max(round((np.prod(self.vol_params.size) / (self.vasc_params.ves_freq[2]**3)) * abs(1 + self.vasc_params.ves_num_scale * np.random.randn())), 0)
            ...
            if self._nodes[c.start].node_type in ['edge', 'surf', 'sfvt']:
                ...
                if conn_length > 0:  # Avoid division by zero
                    ...
                    self._nodes[c.start].pos[2] = min(
                        self._nodes[c.start].pos[2] + z_adjustment,
                        self.vasc_params._depth_surf
                    )
            
            # Adjust end node if it's a surface vessel
            if self._nodes[c.ends].node_type in ['edge', 'surf', 'sfvt']:
                conn_length = len(self._nodes[c.start].conn)  # Using start connections as per MATLAB
                if conn_length > 0:  # Avoid division by zero
                    z_adjustment = np.ceil(c.weight / conn_length)
                    self._nodes[c.ends].pos[2] = min(
                        self._nodes[c.ends].pos[2] + z_adjustment,
                        self.vasc_params._depth_surf
                    )
        ...
        return self._neur_ves.copy(), self._neur_ves_all.copy() if self.vol_params.verbose > 1 else None
    ...
    def _grow_major_vessels(self) -> None:
        ...
        for i in range(self.vasc_params._nsource):
            ...
            tmp_idx = [
                random.random() >= self.vasc_params._size[1]/(self.vasc_params._size[0] + self.vasc_params._size[1]),
                random.random() >= 0.5
            ]
        ...
        surf_ves = np.zeros((int(self.vasc_params._size[0]), int(self.vasc_params._size[1])), dtype=bool)
        for i in range(self.vasc_params._nsource):
        ...
        # Add z-coordinate
        surf_pos = np.column_stack([surf_pos, np.full(self.vasc_params._nsurf, self.vasc_params._depth_surf)])
        ...
        surf_mat[:self.vasc_params._nlinks + self.vasc_params._nsource, :self.vasc_params._nlinks + self.vasc_params._nsource] = np.inf
        ...
        for i in range(self.vasc_params._nlinks + self.vasc_params._nsource, self.vasc_params._nlinks + self.vasc_params._nsource + self.vasc_params._nsurf):
            pos = surf_pos[i]
            is_edge = (pos[0] in [1, self.vasc_params._size[0]]) or (pos[1] in [1, self.vasc_params._size[1]])
        ...
        self.vasc_params._nnodes = self.vasc_params._nlinks + self.vasc_params._nsource + self.vasc_params._nsurf
        for i in range(self.vasc_params._nnodes):
            if self._nodes[i].root >= 0:
                self._nodes[self._nodes[i].root].conn = list(set(self._nodes[self._nodes[i].root].conn + [i]))
        ...
        surf_pos = np.array([node.pos[:2] for node in self._nodes if node.node_type == 'surf']).astype(int)
        while (sum(1 for node in self._nodes if node.node_type == 'sfvt') < self.vasc_params._nvert and np.any(neur_vert[surf_pos[:, 0], surf_pos[:, 1]] == 0)):
            ...
            tmp = np.zeros(self.vasc_params._size[:2], dtype=bool)
            tmp[self._nodes[tmp_idx].pos[0].astype(int), self._nodes[tmp_idx].pos[1].astype(int)] = 1
            neur_vert = np.logical_or(neur_vert, ndimage.binary_dilation(tmp, se))
        ...
        for i in vert_idx:
            ...
            while self._nodes[curr_node].pos[2] < self.vasc_params._size[2] - 1:
                node_pos = self._nodes[curr_node].pos + np.ceil([
                    ...
                    np.random.randn() * self.node_params.varpos,
                    max(self.node_params.varsc * np.random.randn() + self.node_params.lensc, self.node_params.mindist)
                ])
    ...
    def _branch_grow_nodes(
        ...
        while num_it < self.node_params.maxit and border_flag:
            ...
            ves_dist = max(self.node_params.varsc * np.random.randn() + self.node_params.lensc, self.node_params.mindist)
            ...
            test_idxs = np.ravel_multi_index(
                multi_index=(test_subs_padded[0, :].astype(int), test_subs_padded[1, :].astype(int)),
                dims=nv_size[:2]
            )
        ...
        for end_idx in ends:
            ...
            while self._nodes[curr_node].root >= 0:
                ...
                conn_mat[self._nodes[curr_node].num, self._nodes[curr_node].root] = np.sqrt(
                    conn_mat[self._nodes[curr_node].num, self._nodes[curr_node].root]**2 + weight**2
                )
    ...
    def _conn_to_vol(
        ...
        for i in idxs:
            ...
            min_idx = np.maximum(np.min(ves_loc, axis=0) - np.ceil(self._conn[i].weight), [0, 0, 0]).astype(int)
            max_idx = np.minimum(np.max(ves_loc, axis=0) + np.ceil(self._conn[i].weight), self.vasc_params._size - 1).astype(int)
            ves_loc = ves_loc - min_idx

            # Create 3D volume and dilate
            tmp = np.zeros((max_idx - min_idx + 1).astype(int), dtype=bool)
            tmp[ves_loc[:, 0].astype(int), ves_loc[:, 1].astype(int), ves_loc[:, 2].astype(int)] = True
            x, y, z = np.meshgrid(
            ...
            self._neur_ves[min_idx[0]:max_idx[0]+1,
                    min_idx[1]:max_idx[1]+1,
                    min_idx[2]:max_idx[2]+1] = np.logical_or(
                self._neur_ves[min_idx[0]:max_idx[0]+1,
                         min_idx[1]:max_idx[1]+1,
                         min_idx[2]:max_idx[2]+1], tmp
            )
    ...
    def _grow_capillaries(self) -> None:
        ...
        for i in range(len(self._conn)):
            ...
            for pos in tmp_pos:
                ...
                tmp_l = (tmp_l < 0) * (-tmp_l)
                tmp_u = pos + dilrad
                tmp_u = (tmp_u > self.vasc_params._szum - 1) * (tmp_u - (self.vasc_params._szum - 1))
                
                # Extract relevant portion of structural element
                tmp = se[int(tmp_l[0]):se.shape[0]-int(tmp_u[0]),
                ...
                tmp_vol[int(tmp_l[0]):int(tmp_u[0])+1,
                       int(tmp_l[1]):int(tmp_u[1])+1,
                       int(tmp_l[2]):int(tmp_u[2])+1] = np.maximum(
                    tmp_vol[int(tmp_l[0]):int(tmp_u[0])+1,
                           int(tmp_l[1]):int(tmp_u[1])+1,
                           int(tmp_l[2]):int(tmp_u[2])+1], tmp)
        ...
        capp_pos = capp_pos * self.vol_params.res + 1 - np.random.randint(max(1, int(self.vol_params.res)), size=capp_pos.shape)
        
        # Setup connections from diving vessels to capillaries
        nv_vert_conn = np.random.randint(
            0, self.vasc_params._szum[2]//self.vasc_params.ves_freq[2],
        ...
        if len(self.vasc_params._ves_size) < 4:
            self.vasc_params._ves_size = np.append(self.vasc_params._ves_size, self.vasc_params._ves_size[2] * 0)
        ...
        for i, vert_idx in enumerate(vert_idxs):
            ...
            while True:
                tmp_idx = [n for n in self._nodes[ves_idx[-1]].conn 
            ...
            for _ in range(nv_vert_conn[i]):
                ...
                weight = max(1, np.random.normal(self.vasc_params._ves_size[2], self.vasc_params._ves_size[3]))
                self._conn.append(Connection(
                    start=node_idx,
                    ends=tmp_idx,
                    weight=weight,
                    misc='vtcp'
        ...
        vert_conn_idxs = [i for i, n in enumerate(self._nodes) 
                         if self._nodes[i].root != -1 and self._nodes[i].node_type == 'capp']
        capp_conn_idxs = [i for i, n in enumerate(self._nodes) 
                         if self._nodes[i].root == -1 and self._nodes[i].node_type == 'capp']
        ...
        for i in range(self.vasc_params._nvert_sum, self.vasc_params._ncapp):
            for j in range(i + 1, self.vasc_params._ncapp):
                if capp_mat[i, j] < np.inf:
                    ...
                    indices = (xpix[:, 0]).astype(int), (xpix[:, 1]).astype(int), (xpix[:, 2]).astype(int)
        ...
        while len(to_connect) > 0:
            ...
            if np.isnan(self._conn[curr_conn].weight):
                ...
                start_conns = start_conns[
                    (start_conns != curr_conn) & (start_conns != 0)]
                end_conns = end_conns[
                    (end_conns != curr_conn) & (end_conns != 0)]
                ...
                if np.isnan(weight1):
                    if np.isnan(weight2):
                        conn_weight = max(1, np.random.normal(
        ...
        for i in range(self.vasc_params._nconn):
            if self._conn[i].weight is None or np.isnan(self._conn[i].weight):
                self._conn[i].weight = max(1, np.random.normal(
                    self.vasc_params._ves_size[2], self.vasc_params._ves_size[3]))
    ...
    def plot_vasculature(self, save_path: str = None, show: bool = True) -> None:
        ...
        for conn in self._conn:
            if conn.locs is not None:  # Skip connections without location data
                ...
                ax.plot(conn.locs[:, 0], conn.locs[:, 1], conn.locs[:, 2],
        ...
        ax.legend([surface_line, vertical_line, capillary_line],
